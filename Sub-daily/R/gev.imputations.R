...
#Run script
library(parallel)
load("~/PhD/Sub-daily/Results/Alex_map/final/mc.mod6.RData")
...
p.return <- .04 ## Let's look at the 10 year precipitation event
##Z <- impute.value(mc,coord.new, x.new.loc,x.new.scale,x.new.shape,p.return)
S <- dim(mc$chain.loc)[1]
cores.use <- 10
l <- mclapply(1:S, "impute.value.single",mc, coord.new, x.new.loc, x.new.scale, x.new.shape, p.return, mc.cores = cores.use, mc.silent = FALSE)
...
save(Z, file = paste("~/PhD/Sub-daily/Results/Alex_map/final/M",rl,model,".RData",sep=""))
