...
max.dist <- max(iDist(coord.norm))
bins <- 1000

vario <- variog(coords=coord.norm, data=scale, uvec= seq(0, max.dist, l = 1000),max.dist=max.dist)
...
vario <- variog(coords=coord.norm, data=scale, uvec= seq(0, max.dist, l = 5000),max.dist=max.dist)
...
plot(vario)
lines(fit.vario)
abline(h=sc.nugget, col="blue")##nugget
abline(h=sc.sill, col="green")##sill
abline(v=sc.eff.range, col="red3")##effective range 
#dev.off()
