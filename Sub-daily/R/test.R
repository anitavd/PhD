...
##########################################
n<-1
mc$chain.loc <- mc$chain.loc[n:(n+9),]
mc$chain.scale <- mc$chain.scale[n:(n+9),]
mc$chain.shape <- mc$chain.shape[n:(n+9),]

S <- dim(mc$chain.loc)[1]

##########################################

l <- mclapply(1:S, "impute.value.single",mc, coord.new, x.new.loc, x.new.scale, x.new.shape, p.return, mc.cores = 1, mc.silent = FALSE)
