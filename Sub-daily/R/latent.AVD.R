latent.AVD <- function (data, coord, cov.mod = "powexp", loc.form, scale.form, 
    ...
    thin = 1, burn.in = 0) 
{
    ...
    for (i in 1:length(hyper)) if (!all(names(hyper[[i]]) %in% 
        c("loc", "scale", "shape")) || length(hyper[[i]]) != 
        3) 
    ...
    for (i in 1:3) if (length(prop[[i]]) != 3) 
    ...
    if (is.null(start$beta$loc) || is.null(start$beta$scale) || 
        is.null(start$beta$shape)) 
        stop("''start$beta'' must be a named list with named components 'loc', 'scale' and 'shape'")
    for (i in 1:3) if ((length(start$sills) != 3) || (length(start$ranges) != 
        3) || (length(start$smooths) != 3)) 
    ...
    else covariables <- data.frame(coord, marg.cov)
    ...
    for (i in 1:3) if ((length(hyper$betaMeans[[i]]) != n.beta[i]) || 
        (any(dim(hyper$betaIcov[[i]]) != c(n.beta[i], n.beta[i])))) 
        stop("The hyper parameters for the regression parameters doesn't match")
    for (i in 1:3) if (length(start$beta[[i]]) != n.beta[i]) 
        stop("The starting values for the regression parameters doesn't match")
    for (k in 1:ncol(data)) {
	if(k==1) GEVmles <- gevmle(data[,k][!is.na(data[,k])])
	else GEVmles <- rbind(GEVmles,gevmle(data[,k][!is.na(data[,k])]))
    }
    ...
    #.C is a C-function (in BLAS routines)
    temp <- .C("latentgev", as.integer(n), as.double(data), as.integer(n.site), 
        ...
        as.double(distMat), as.double(c(loc.dsgn.mat, scale.dsgn.mat, 
            shape.dsgn.mat)), as.integer(n.beta), as.double(unlist(start$beta)), 
        as.double(start$sills), as.double(start$ranges), as.double(start$smooths), 
        as.double(GEVmles), as.double(unlist(hyper$sills)), as.double(unlist(hyper$ranges)), 
        as.double(unlist(hyper$smooths)), as.double(unlist(hyper$betaMeans)), 
        as.double(unlist(hyper$betaIcov)), as.double(prop$gev), 
        as.double(prop$ranges), as.double(prop$smooths), chain.loc = chain.loc, 
        chain.scale = chain.scale, chain.shape = chain.shape, 
    ...
    names(acc.rates) <- names(ext.rates) <- c("gev:loc", "gev:scale", 
        "gev:shape", "range:loc", "range:scale", "range:shape", 
    ...
    colnames(chain.loc) <- c(paste("lm", 1:n.loccoeff, sep = ""), 
        "sill", "range", "smooth", paste("loc", 1:n.site, sep = ""))
    colnames(chain.scale) <- c(paste("lm", 1:n.scalecoeff, sep = ""), 
        "sill", "range", "smooth", paste("scale", 1:n.site, sep = ""))
    colnames(chain.shape) <- c(paste("lm", 1:n.shapecoeff, sep = ""), 
        "sill", "range", "smooth", paste("shape", 1:n.site, sep = ""))
    mcmc <- list(chain.loc = chain.loc, chain.scale = chain.scale, 
        ...
        hyper = hyper, cov.mod = cov.mod, burn.in = burn.in, 
        thin = thin, data = data, coord = coord, marg.cov = marg.cov, 
