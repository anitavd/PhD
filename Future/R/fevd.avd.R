  shapePriorNormal <- function (x, m, s) 
  {
      if (!is.element("shape", colnames(x))) 
      stop("shapePriorNormal: invalid model for this prior penalty function.")
    xi <- x[, "shape"]
    res <- -sum(dnorm(x = xi, mean = m, sd = s, log = TRUE), 
                na.rm = TRUE)
    return(res)
  }

fevd.avd <- function (x, data, threshold = NULL, threshold.fun = ~1, location.fun = ~1, 
    ...
    iter = 9999, weights = 1, blocks = NULL, verbose = FALSE) 
{
    ...
    if (method == "MLE" && !is.null(priorFun)) 
        ...
        if (is.element(type, c("GEV", "GP", "PP"))) {
            priorFun <- "shapePriorNormal"
            if (is.null(priorParams)) 
                priorParams <- list(m = 0, s = 1)
        }
        else {
    ...
    if (is.element(method, c("mle", "gmle"))) {
        if (use.phi) 
            init.s <- c(initial$location, initial$log.scale, 
                initial$shape)
