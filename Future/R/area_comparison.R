...
#Read AM
am.3h.obs <- read.table(paste("~/PhD//Future//Data//Obs/",area,"area_AM_pr3h_19712014.txt",sep=""),header=T)

#6 stations and 6 grid cells for analysis.
nc <- nc_open(paste("~/PhD//Future//Data//Eur-11//DMI/",area,"-AM-EUR11pr3h_ICHEC_DMI_rcp45_1971-2014.nc",sep=""))
am.3h.dmi <- ncvar_get(nc,"pr")*60*60
nc_close(nc)

nc <- nc_open(paste("~/PhD//Future//Data//Eur-11/IPSL/",area,"-AM-EUR11pr3h_WRF_IPSL_rcp45_1971-2014.nc",sep=""))
am.3h.ipsl <- ncvar_get(nc,"pr")*60*60*3
nc_close(nc)

nc <- nc_open(paste("~/PhD//Future//Data//Eur-11/KNMI/",area,"-AM-EUR11pr3h_ICHEC_KNMI_rcp45_1971-2014.nc",sep=""))
am.3h.knmi <- ncvar_get(nc,"pr")*60*60*3
nc_close(nc)

nc <- nc_open(paste("~/PhD//Future//Data//Eur-11/SMHI/ICHEC/",area,"-AM-EUR11pr3h_ICHEC_SMHI_rcp45_1971-2014.nc",sep=""))
am.3h.smhi.i <- ncvar_get(nc,"pr")*60*60*3
nc_close(nc)

nc <- nc_open(paste("~/PhD//Future//Data//Eur-11/SMHI/CNRM/",area,"-AM-EUR11pr3h_CNRM_SMHI_rcp45_1971-2014.nc",sep=""))
am.3h.smhi.c <- ncvar_get(nc,"pr")*60*60*3
nc_close(nc)

#Read WF
wf.3h.obs <- read.table(paste("~/PhD//Future//Data//Obs/",area,"area_WF_pr3h_19712014.txt",sep=""),header=T)

nc <- nc_open(paste("~/PhD//Future//Data//Eur-11//DMI/",area,"-WF-EUR11pr3h_ICHEC_DMI_rcp45_1971-2014.nc",sep=""))
wf.3h.dmi <- ncvar_get(nc,"pr")
nc_close(nc)

nc <- nc_open(paste("~/PhD//Future//Data//Eur-11/IPSL/",area,"-WF-EUR11pr3h_WRF_IPSL_rcp45_1971-2014.nc",sep=""))
wf.3h.ipsl <- ncvar_get(nc,"pr")
nc_close(nc)

nc <- nc_open(paste("~/PhD//Future//Data//Eur-11/KNMI/",area,"-WF-EUR11pr3h_ICHEC_KNMI_rcp45_1971-2014.nc",sep=""))
wf.3h.knmi <- ncvar_get(nc,"pr")
nc_close(nc)

nc <- nc_open(paste("~/PhD//Future//Data//Eur-11/SMHI/ICHEC/",area,"-WF-EUR11pr3h_ICHEC_SMHI_rcp45_1971-2014.nc",sep=""))
wf.3h.smhi.i <- ncvar_get(nc,"pr")
nc_close(nc)

nc <- nc_open(paste("~/PhD//Future//Data//Eur-11/SMHI/CNRM/",area,"-WF-EUR11pr3h_CNRM_SMHI_rcp45_1971-2014.nc",sep=""))
wf.3h.smhi.c <- ncvar_get(nc,"pr")
nc_close(nc)

#Read WT
wi.3h.obs <- read.table(paste("~/PhD//Future//Data//Obs/",area,"area_WI_pr3h_19712014.txt",sep=""),header=T)
wt.3h.obs <- wi.3h.obs*wf.3h.obs
wt.3h.obs[,1] <- wi.3h.obs[,1]

nc <- nc_open(paste("~/PhD//Future//Data//Eur-11//DMI/",area,"-WT-EUR11pr3h_ICHEC_DMI_rcp45_1971-2014.nc",sep=""))
wt.3h.dmi <- ncvar_get(nc,"pr")*60*60
nc_close(nc)

nc <- nc_open(paste("~/PhD//Future//Data//Eur-11/IPSL/",area,"-WT-EUR11pr3h_WRF_IPSL_rcp45_1971-2014.nc",sep=""))
wt.3h.ipsl <- ncvar_get(nc,"pr")*60*60*3
nc_close(nc)

nc <- nc_open(paste("~/PhD//Future//Data//Eur-11/KNMI/",area,"-WT-EUR11pr3h_ICHEC_KNMI_rcp45_1971-2014.nc",sep=""))
wt.3h.knmi <- ncvar_get(nc,"pr")*60*60*3
nc_close(nc)

nc <- nc_open(paste("~/PhD//Future//Data//Eur-11/SMHI/ICHEC/",area,"-WT-EUR11pr3h_ICHEC_SMHI_rcp45_1971-2014.nc",sep=""))
wt.3h.smhi.i <- ncvar_get(nc,"pr")*60*60*3
nc_close(nc)

nc <- nc_open(paste("~/PhD//Future//Data//Eur-11/SMHI/CNRM/",area,"-WT-EUR11pr3h_CNRM_SMHI_rcp45_1971-2014.nc",sep=""))
wt.3h.smhi.c <- ncvar_get(nc,"pr")*60*60*3
nc_close(nc)

###############################################################################################################################

#Compute GEV from combines series, UPDATE!!!!!!!!!!!!!
...
############################################################################################################
#AM, WF, WI, WT, -plots
#COMBINE ALL DATA (might induce problems with correlation, but good for seeing the spread)
...
png(paste("~/PhD//Future//Figs/",area,"area_AM_comparison.png",sep=""),width=820,height=700)
boxplot(AM,col=col,ylab="Annual maximum 3-hr precipitation [mm]")
dev.off()

png(paste("~/PhD//Future//Figs/",area,"area_WF_comparison.png",sep=""),width=820,height=700)
boxplot(WF,col=col,ylab="Frequency of 3-hr precipitation events > 0.5 mm, MJJAS")
dev.off()

png(paste("~/PhD//Future//Figs/",area,"area_WT_comparison.png",sep=""),width=820,height=700)
boxplot(WT,col=col,ylab="Total precipitation for 3-hr events > 0.5 mm, MJJAS")
dev.off()

png(paste("~/PhD//Future//Figs/",area,"area_WI_comparison.png",sep=""),width=820,height=700)
...
#EXTRACT mean WF,WI,WT and max AM for each year (maybe skip this since we are looking at real averages of stations within same gric cells?)
AM <- list(Obs=as.numeric(as.matrix(apply(am.3h.obs[,-1],1,max,na.rm=T))),DMI=as.vector(am.3h.dmi),IPSL=as.vector(am.3h.ipsl),KNMI=as.vector(am.3h.knmi),SMHI.I=as.vector(am.3h.smhi.i),SMHI.C=as.vector(am.3h.smhi.c))
WF <- list(Obs=as.numeric(as.matrix(wf.3h.obs[,-1])),DMI=as.vector(wf.3h.dmi),IPSL=as.vector(wf.3h.ipsl),KNMI=as.vector(wf.3h.knmi),SMHI.I=as.vector(wf.3h.smhi.i),SMHI.C=as.vector(wf.3h.smhi.c))
WT <- list(Obs=as.numeric(as.matrix(wt.3h.obs[,-1])),DMI=as.vector(wt.3h.dmi),IPSL=as.vector(wt.3h.ipsl),KNMI=as.vector(wt.3h.knmi),SMHI.I=as.vector(wt.3h.smhi.i),SMHI.C=as.vector(wt.3h.smhi.c))
WI <- list(Obs=as.numeric(as.matrix(wi.3h.obs[,-1])),DMI=as.vector(wt.3h.dmi/wf.3h.dmi),IPSL=as.vector(wt.3h.ipsl/wf.3h.ipsl),KNMI=as.vector(wt.3h.knmi/wf.3h.knmi),SMHI.I=as.vector(wt.3h.smhi.i/wf.3h.smhi.i),SMHI.C=as.vector(wt.3h.smhi.c/wf.3h.smhi.c))
